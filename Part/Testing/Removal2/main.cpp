...
{
  const char* filename[] = { "newPartProb", 
	                     "ibm01", "ibm02", "ibm03",
	                     "ibm04", "ibm05", "ibm06",
	                     "ibm07", "ibm08", "ibm09",
	                     "ibm10", "ibm11", "ibm12",
  ...
  for (int i=0; i<1; i++) {
    ...
    LayerAssignmentServ las(H);
    las.doAssignment();
    string dotFile = "Nets/" + string(filename[i]) + ".dot";
    las.writeDot(dotFile.c_str());
    ...
    //xxx LayerAssignmentServ las(*H1);
    //xxx las.doAssignment();
    //xxx string dotFile = "Nets/" + string(filename[i]) + ".dot";
    //xxx las.writeDot(dotFile.c_str());
