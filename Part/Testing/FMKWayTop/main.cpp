...
int main(int argc, const char* argv[])
{
  ...
  for (int i=0; i<18; i++) {
    Netlist H;
    //xxx string netDFile = "Nets/" + string(filename[i]) + ".netD";
    //xxx if (!H.readNetD(netDFile.c_str())) {
    //xxx   cout << "Could not read .netD file " << netDFile << endl;
    //xxx   continue;
    //xxx }    
    ...
    //xxx   seed48(&seed);
    //xxx   FMPartTmpl<FMKWayPartCore, FMKWayGainMgr2> P2(H,numOfParts,1);  
